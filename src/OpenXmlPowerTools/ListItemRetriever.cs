...
Copyright (c) Microsoft Corporation 2010-2013.
...
namespace OpenXmlPowerTools
{
    public partial class WmlDocument : OpenXmlPowerToolsDocument
    {
        ...
        {
            var listItem = ListItemRetriever.RetrieveListItem(this, paragraph, bulletReplacementString);
            return listItem;
        }
    }
    ...
    public class ListItemRetriever
    {
            ...
            XElement paragraph)
        {
            ...
            {
                var paragraphStyleElement = styles
                    .Root
                    .Elements(W.style)
                    .FirstOrDefault(s => (string)s.Attribute(W.type) == "paragraph" &&
                        ((string)s.Attribute(W._default) == "1" || ((string)s.Attribute(W._default)).ToLower() == "true")); // TODO NEED BETTER CONVERSION TO BOOLEAN
                if (paragraphStyleElement != null)
                    paragraphStyle = (string)paragraphStyleElement.Attribute(W.styleId);
            }
        ...
        {
            IEnumerable<XElement> paragraphsBeforeSelf = paragraph
                .ParagraphsBeforeSelfForListItemRetriever();
            int level;
            ListItemInfo listItemInfo = GetListItemInfo(numbering, styles, paragraph);
            ...
            for (int indentationLevel = 0; indentationLevel <= level; ++indentationLevel)
            {
                ...
                if (paragraphWithSameAbstractNumId != null)
                {
                    ...
                    if (pLevelNumbers != null && pLevelNumbers.LevelNumbersArray.Length > indentationLevel)
                    {
        ...
        // todo this could potentially be improved.  This is really an iterative algorithm -
        // - First look to see if in override
        //     If so then see if exists related level, and return
        //     If no related level, go to abstractNum, see if exists related level, and return
        //     If no related level, get style via numStyleLink, get numId, get w:num, see if exists related level, and return
        //     If no related level, go to abstractNum, see if exists related level, and return
        //     If no related level, get style via numStyleLink, get numId, get w:num, see if exists related level, and return
        //     etc.
        // I have never seen an instance of going through the loop more than once, but suspect it is possible.
        private static XElement GetRelatedLevel(XElement lvl, int level, XDocument styles)
        {
            XElement thisLvl = lvl;
            XElement parent = thisLvl.Parent;
            XElement newLvl;
            if (parent.Name == W.lvlOverride)
            {
                ...
                int abstractNumId = (int)parent.Parent.Elements(W.abstractNumId)
                    .Attributes(W.val).FirstOrDefault();
                XElement abstractNum = thisLvl.Ancestors().Last().Elements(W.abstractNum)
                ...
                if (newLvl != null)
                    return newLvl;
                var numStyleLink = (string)abstractNum.Elements(W.numStyleLink).Attributes(W.val).FirstOrDefault();
                if (numStyleLink == null)
                    return null;
                int? numId = (int?)styles
                    .Root
                    .Elements(W.style)
                    .Where(s => (string)s.Attribute(W.type) == "numbering" && (string)s.Attribute(W.styleId) == numStyleLink)
                    .Elements(W.pPr)
                    .Elements(W.numPr)
                    .Elements(W.numId)
                    .Attributes(W.val)
                ...
                if (num == null)
                    return null;
                newLvl = num.Elements(W.lvlOverride).Elements(W.lvl)
                    .Where(e => (int)e.Attribute(W.ilvl) == level).FirstOrDefault();
                if (newLvl != null)
                    return newLvl;
        ...
        private static string FormatListItem(XElement lvl, int[] levelNumbers,
            string lvlText, string bulletReplacementString, XDocument styles)
        {
            ...
            string listItem = formatTokens.Select((t, l) =>
            {
                ...
                string levelText;
                XElement rlvl = GetRelatedLevel(lvl, indentationLevel, styles);
        ...
        public static string RetrieveListItem(WordprocessingDocument wordDoc,
            XElement paragraph, string bulletReplacementString)
        {
            if (paragraph.Descendants(W.r).FirstOrDefault() == null ||
                !paragraph.Descendants(W.r).Elements(W.t).Any())
                return null;
            ...
            if (listItemInfo.IsListItem)
            {
                ...
                string listItem = FormatListItem(listItemInfo.Lvl, levelNumbers, lvlText,
                    bulletReplacementString, styles);
                return listItem;
            }
            return null;
        }
    ...
    public static class ListItemRetrieverExtensions
    {
        public static IEnumerable<XElement> ParagraphsBeforeSelfForListItemRetriever(this XElement paragraph)
        {
            var currentInTree = paragraph;
            while (true)
            {
                var elementsBeforeSelf = currentInTree.ElementsBeforeSelfReverseDocumentOrder();
                foreach (var element in elementsBeforeSelf)
                {
                    if (element.Name == W.p)
                    {
                        var run = element.Descendants(W.r).FirstOrDefault();
                        if (run != null && run.Element(W.t) != null)
                            yield return element;
                    }
                    if (element.Name == W.tbl || element.Name == W.tr || element.Name == W.tc)
                    {
                        var descendants = element.DescendantsTrimmed(W.txbxContent)
                            .Where(d => 
                            {
                                if (d.Name != W.p)
                                    return false;
                        ...
                        foreach (var d in descendants.Reverse())
                            yield return d;
                    }
                }
                currentInTree = currentInTree.Parent;
                var n = currentInTree.Name;
                if (n == W.body ||
                    n == W.txbxContent ||
                    n == W.hdr ||
                    n == W.ftr ||
